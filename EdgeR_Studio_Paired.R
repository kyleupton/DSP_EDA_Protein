...
# ); 
option_list = list(
  make_option(c("-c", "--configPath"), type="character", default='/Users/upton6/Library/CloudStorage/OneDrive-QueenslandUniversityofTechnology/Documents/notebooks/Nanostring/Adams_Bray_EOI', 
              help="dataset file name", metavar="character"),
  make_option(c("-d", "--rootdir"), type="character", default='/Users/upton6/Documents/Nanostring/projects/Adams_bray_EOI/DSP_Output_Files/', 
              help="dataset file name", metavar="character"),
  make_option(c("-n", "--normpath"), type="character", default='Normalisation/NSNorm', 
              help="dataset file name", metavar="character"),
  make_option(c("-f", "--file"), type="character", default='NanoStringNorm_25_none_none_housekeeping.geo.mean.csv', 
              help="dataset file name", metavar="character"),
  make_option(c("-e", "--exportdir"), type="character", default='EdgeR/NSNorm_25', 
              help="dataset file name", metavar="character"),
  make_option(c("-r", "--runname"), type="character", default=NULL, 
              help="dataset file name", metavar="character"),
  make_option(c("-i", "--sampleinfo"), type="character", default='sampleInfo_with_wells_dropped_paired.csv', 
              help="dataset file name", metavar="character")
); 
...
make_Paired_Design <- function(Subject, Treat){
  #### Is this the best design set-up? Double check the use of 0 vs other options
...
setwd(configDir)
config = readLines("EdgeR_Config_Paired.txt")
...
Subject <- factor(targets$Patient2)
Treat <- factor(targets$Broad_classification, levels=c("Primary","Secondary"))
design <- model.matrix(~Subject+Treat)
...
pdfName = paste("MD_plot_", "paired_Broad_Classification", ".pdf", sep="")
filename = paste("MD_plot_", "paired_Broad_Classification", ".csv", sep="")
...
#   design <- make_Paired_Design(targets$Patient2,group)
